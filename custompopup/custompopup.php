<?php
/*
 * Módulo simples feito num dia de muita inspiração. Não revenda e não distribua sem autorização do desenvolvedor, mais conhecido como EU.
*/
...
class CustomPopup extends Module
{
    ...
    public function installFixtures()
    {
        ...
        Db::getInstance()->insert('responsive_popup_pages', array(
             'id_page' => 'Homepage',
             'id_shop' => $shopid,
             'enabled' => '0'
        ));
...
<script src="../modules/custompopup/js/bootbox.js"></script>
<script>
    ...
    public function displayTabs()
    {
        ...
        <div role="tabpanel">
            ...
            <ul class="nav nav-tabs nav-tabs-sticky" role="tablist">
                <li role="presentation" class="active"><a href="#settings" aria-controls="home" role="tab" data-toggle="tab"><i class="icon-cogs"></i>&nbsp;&nbsp;&nbsp;'.$this->l('Main Settings').'</a></li>
                <li role="presentation"><a href="#customizestyle" aria-controls="profile" role="tab" data-toggle="tab"><i class="icon-pencil"></i>&nbsp;&nbsp;&nbsp;'.$this->l('Customize Style').'</a></li>
                <li role="presentation"><a href="#display" aria-controls="profile" role="tab" data-toggle="tab"><i class="icon-eye-open"></i>&nbsp;&nbsp;&nbsp;'.$this->l('Display on pages').'</a></li>
                <li role="presentation"><a href="#about" aria-controls="profile" role="tab" data-toggle="tab"><i class="icon-info-circle"></i>&nbsp;&nbsp;&nbsp;'.$this->l('About').'</a></li>
            </ul>
        ...
        <table>
        <tr><td style="text-align:right;font-weight:bold;">'.$this->l('Name').':</td> <td>&nbsp;&nbsp;Custom popup notification</td></tr>
        <tr><td style="text-align:right;font-weight:bold;">'.$this->l('Release date').':</td><td>&nbsp;&nbsp;15.02.2016</td></tr>
        <tr><td style="text-align:right;font-weight:bold;">'.$this->l('Module version').':</td> <td>&nbsp;&nbsp;1.0</td></tr>
        <tr><td style="text-align:right;font-weight:bold;">'.$this->l('PrestaShop compatible').':</td><td>&nbsp;&nbsp;1.6</td></tr>
        <tr><td style="text-align:right;font-weight:bold;">'.$this->l('Module website').':</td><td>&nbsp;
        <a href="http://prestacraft.com/free-modules/home/8-custom-popup-notification.html" target="_blank">'.$this->l('Click').'</a></td></tr>
        </table>
...
<br /><br />
        ...
        <br />
        <br />'.$this->l('Made with').' <i class="icon-heart"></i> '.$this->l('by').' <a href="'.$this->l('http://prestacraft.com').'" target="_blank">PrestaCraft</a>.
<br /><br /><br />
...
<img alt="" border="0" src="https://www.paypalobjects.com/pl_PL/i/scr/pixel.gif" width="1" height="1">
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('saveCustomPopup'))
        {
            ...
            $Validation->validate($this->l('Cookie length'),Tools::getValue('CUSTOMPOPUP_COOKIE'),array(
                'isnumber' => 1
            ));
            ...
            if($Validation->getAllErrors())
            {
                $errors = array();
                  ...
                  foreach($Validation->getAllErrors() as $key => $value)
                  {
        ...
        {
            $Validation->validate($this->l('Popup color'),Tools::getValue('CUSTOMPOPUP_COLOR'),array(
                'ishex' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Background color'),Tools::getValue('CUSTOMPOPUP_BACK_COLOR'),array(
                'ishex' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Content padding'),Tools::getValue('CUSTOMPOPUP_PADDING'),array(
                'isnumber' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Content top padding'),Tools::getValue('CUSTOMPOPUP_TOP_PADDING'),array(
                'isnumber' => 1,
                'notempty' => 1
            ));
        ...
        if (Tools::isSubmit('saveStyleClose'))
        {
            ...
            $Validation->validate($this->l('Color'),Tools::getValue('CUSTOMPOPUP_BUTTON_COLOR'),array(
                'ishex' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Hover color'),Tools::getValue('CUSTOMPOPUP_BUTTON_HOVER_COLOR'),array(
                'ishex' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Size'),Tools::getValue('CUSTOMPOPUP_BUTTON_SIZE'),array(
                'isnumber' => 1,
                'notempty' => 1
            ));

            $Validation->validate($this->l('Top padding'),Tools::getValue('CUSTOMPOPUP_BUTTON_TOP_PADDING'),array(
                'isnumber' => 1,
                'notempty' => 1
            ));
            ...
            {
                Configuration::updateValue('CUSTOMPOPUP_BUTTON_HOVER_COLOR',Tools::getValue('CUSTOMPOPUP_BUTTON_HOVER_COLOR'));
            }
            ...
            {
                Configuration::updateValue('CUSTOMPOPUP_BUTTON_TOP_PADDING',Tools::getValue('CUSTOMPOPUP_BUTTON_TOP_PADDING'));
            }
    ...
    public function renderSettings()
    {
        ...
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&pos=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
    ...
    public function renderCustomizeStyle()
    {
        ...
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&pos=2&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'uri' => $this->getPathUri(),
    ...
    public function renderDisplay()
    {
        ...
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&pos=3&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'uri' => $this->getPathUri(),
    ...
    public function hookDisplayHome($params)
    {
        ...
        $assign = array('popup_cookie' => Configuration::get('CUSTOMPOPUP_COOKIE'),
    ...
    public function hookDisplayHeader()
    {
        ...
        $this->context->controller->addJS($this->_path.'js/jquery.popup.min.js', 'all');
        $this->context->controller->addCSS($this->_path.'css/popup.css', 'all');
    }
    ...
    protected function updateDbValues($array)
    {
        ...
        {
           Db::getInstance()->update('responsive_popup',
    ...
    {
        Db::getInstance()->update('responsive_popup_pages',
            array('enabled' => $value),
            'id_page="'.$field.'";');
    }
